  ...
  Renderers.matching = function (ex, ctx) {
    ...
    right.forEach((r, i) => {
      ...
      card.addEventListener('click', () => {
        ...
        const lCard = colL.querySelector(`[data-id="${selectedLeft}"]`);
        if (lCard) { lCard.classList.add('matched'); lCard.classList.remove('selected'); lCard.textContent = lCard.textContent + ' ✓'; }
